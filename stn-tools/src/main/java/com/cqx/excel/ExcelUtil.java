...
//    private final static Logger logger = Logger.getLogger(ExcelUtil.class);
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                try {
                    ...
                    if (excelExport.containSpecialField(fieldName)) {
                        textValue = excelExport.getSpecialFieldValue(fieldName, value);
    ...
    public static void excelImport(File file, String fileName, IExcelConsumer consumer) throws IOException {
        Workbook workbook = createWorkBook(fileName, new FileInputStream(file));
        consumer.importExcel(workbook);
        return;
    }

    public static void excelImport(File file, String fileName, final IExcelConsumer[] consumers) throws IOException {
        Workbook workbook = createWorkBook(fileName, new FileInputStream(file));

        for (IExcelConsumer consumer : consumers) {
            consumer.importExcel(workbook);
        }
    }
    ...
    public static int getCellIntValue(Cell cell, int defaultInt) {
        try {
